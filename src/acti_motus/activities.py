...
class Activities:
    ...
    def _compute(
        ...
        *,
        ...
        if isinstance(trunk, pd.DataFrame) and not trunk.empty:
            activities = Trunk(
                orientation=self.orientation, config=self.config
            ).compute_activities(trunk, activities, references=references)
            logger.info("Detected activities for trunk.")

        if isinstance(calf, pd.DataFrame) and not calf.empty:
            activities = Calf(orientation=self.orientation).compute_activities(
                calf, activities
            )
        ...
        if isinstance(arm, pd.DataFrame) and not arm.empty:
            activities = activities.join(
                Arm(orientation=self.orientation).compute_activities(arm), how="left"
            )
    ...
    def _map_activities(
        ...
        if series.isna().all():
            raise ValueError(
                "No valid activities found in the series. Please check the input data."
            )
    ...
    def to_sens(
        ...
        for value in values:
            if value not in origin.columns:
                df[value] = (
                    0  # If column is missing, fill with 0. Usually because not all sensors are present
                )
            else:
                df[value] = origin[value]

        df[values] = df[values].fillna(0)  # Fill NaN values with 0
        df["activity"] = self._map_activities(origin["activity"], "text").astype(
            np.int16
    ...
    def compute_sens(
        ...
        df, references = self.compute(
            thigh=thigh, trunk=trunk, calf=calf, arm=arm, references=references
        )
