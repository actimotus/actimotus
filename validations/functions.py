...
MAP_THIGH_TRUNK = {
    ...
    'bicycle (sit)': 'bicycle',
    'lie': 'lie',
    'bicycle (sit, inactive)': 'bicycle',
    'bicycle (stand)': 'bicycle',
    'run': 'run',
    'bicycle (stand, inactive)': 'bicycle',
}
...
MAP_THIGH = {
    ...
    'bicycle (dynamic)': 'bicycle',
    'bicycle (static)': 'bicycle',
...
def process_motus(
    ...
    path = Path(folder)
    files = (path / 'raw_thigh').glob('*.parquet')
    ...
    for thigh in files:
        ...
        if trunk_sensor:
            trunk = thigh.parent.parent / 'raw_trunk' / thigh.name
